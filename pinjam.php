...
if (!isset($_SESSION['user_id'])) {
    echo "User ID tidak ditemukan di session!";
    exit;
}
...
$query_user = "SELECT fullname FROM `table_ss` WHERE id_user = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("INSERT INTO proses_pinjam (id, email, nama, barang, tanggal_pinjam, tanggal_kembali, keterangan) VALUES (?, ?, ?, ?, ?, ?, ?)");
    if ($stmt === false) {
        die('Prepare statement gagal: ' . $conn->error);
    }

    $stmt->bind_param("issssss", $user_id, $email, $fullname, $barang, $tanggal_pinjam, $tanggal_kembali, $keterangan);
...
<body>
    ...
    <form method="post" action="">        
        ...
        <p><a href="tambahBarang.php">klik disini untuk lihat data barang anda</a></p>
    </form>
</body>
</html>
